...
const UploadXlsxToXml = () => {
    ...
    let [xmlSchema, setXmlSchema] = useState();
    ...
    const onInputFileFrom = (data) => {
        // data.preventDefault();
        handleFileInputErrors(data, 'xlsx', fileFromErrorClass, setFileFromErrorClass, setIsFileFromLegit);
    };

    const onInputFileTo = (data) => {
        // data.preventDefault()
        handleFileInputErrors(data, 'xsd', fileToErrorClass, setFileToErrorClass, setIsFileToLegit);
    };

    const handleFileInputErrors = (inputData, allowedExtension, fileErrorClass, setFileErrorClass, setIsFileLegit) => {
    ...
    const onSubmit = async (data) => {
        ...
        fetch("http://127.0.0.1:8080/api/xml/preview", requestOptions)
            .then(response => {
            ...
            .then(result => {
                ...
                console.log(result);
                if (status === 200) {
                    let firstFileColumnsSource = result['headers'];
                    let secondFileColumnsSource = result['endpoints'];
                    let firstFileExamples = result['examples'];
                    let schema = result['schema'];

                    let firstFileColumns = firstFileColumnsSource
                        .flat()
                        .map((columnFromName, index) => [columnFromName, `from-${index}`]);
                    let secondFileColumns = secondFileColumnsSource.map(value => value.join('.'))
                        .flat()
                        .map((columnFromName, index) => [columnFromName, `to-${index}`]);

                    let firstFileColumnsWithExamples = zip(firstFileColumnsSource, firstFileExamples);

                    console.log(firstFileColumns);
                    console.log(secondFileColumns);

                    console.log(firstFileColumnsWithExamples);

                    setDocColumns([firstFileColumns, secondFileColumns]);
                    setZipped(firstFileColumnsWithExamples);
                    setXmlSchema(schema);
        ...
        <div className={css.page}>
            <div className={css.upload}>
                <div className={css.textblock}>
                    <h1 className={css.header}>Конвертация <br/> XLSX в XML</h1>
                ...
                <form onSubmit={onSubmit} className={css.form}>
                    ...
                    <div className={css.file_button_container}>
                        <label htmlFor="source_file"
                               className={css.file_input_label}
                    ...
                    <div className={css.file_button_container}>
                        <label htmlFor="template_file"
                               ...
                               onDrop={onDropHandler}
                               onDragOver={onDragEnterAndOverHandler}>
                            Выбрать .xsd структуру
                        </label>
                        <div className={css.file_input_data}>
                            <p className={css.file_input_description}
                               style={
                                   {
                                       display: `${isFileToLegit ? 'none' : 'block'}`
                                   }
                               }>или перетащите файл
                                сюда</p>
                            <div className={css.arrow_block} style={
                                {
                                    display: `${isFileToLegit ? 'block' : 'none'}`
                                }
                            }>
                                <img className={css.arrow_img} src={arrowOk}></img>
                            </div>
                    ...
                    <p className={fileToErrorClass[0]}>Ошибка! Файл имеет неверное расширение. Необходимо .xsd</p>
...
export default UploadXlsxToXml;